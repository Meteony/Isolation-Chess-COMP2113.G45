...
namespace scenes::main_menu {

// Main-menu intents. main.cpp translates these into concrete scene calls instead
// of letting the launcher depend on live-match, replay, or netplay startup code.
...
// Returned by runLauncher() when the player chooses something that leaves the menu.
...
inline std::string truncateToWidth(const std::string& s, int width) {
  if (width <= 0) return "";
...
inline bool parseIntStrict(const std::string& s, int& outValue) {
...
inline bool isValidServerIp(const std::string& ip) {
  return ip == "localhost" || isValidIpv4(ip);
}

inline bool isValidServerPort(int port) { return port >= 1 && port <= 65535; }

inline bool isValidTag(const std::string& tag) {
...
inline bool isValidRoomCode(const std::string& room) {
...
inline void drawFrameBox(int top, int left, int height, int width,
...
inline void drawScrollbar(int top, int col, int height, int visibleTop,
...
*/
inline std::vector<std::string> logoLinesA() {
  return {
      "                                                ",
      "                             ______  _      -   ",
      "                            __ _/ /_(_▒▒▒▒ _░░___",
...
inline std::vector<std::string> logoLinesB() {
  return {
      "  ________            ______",
...
inline void drawLogoArea(const Layout& layout) {
...
inline void drawInfoBox(const Layout& layout, const Settings& settings,
...
inline std::vector<MenuEntry> makeMainEntries() {
...
inline std::vector<MenuEntry> makeSettingsEntries(const Settings& settings) {
...
inline std::vector<MenuEntry> makeReplayEntries(
...
inline int firstSelectable(const std::vector<MenuEntry>& entries) {
...
inline int moveSelection(const std::vector<MenuEntry>& entries, int current,
                  int delta) {
...
inline void renderMenuList(const Layout& layout, const std::vector<MenuEntry>& entries,
...
inline void clearRect(int top, int left, int height, int width) {
...
inline std::optional<std::string> promptLineInput(const std::string& title,
...
inline void showMessageBox(const Layout& layout, const std::string& title,
...
// Keeps replay preview state local to the launcher instead of leaking it into main.cpp.
struct PreviewState {
...
inline void redrawPreviewBorder(const Layout& layout, FocusPane focus) {
  drawFrameBox(layout.middleTop, layout.rightLeft, layout.previewRows,
               layout.previewCols, "", focus == FocusPane::Replay);
}

inline void drawPreview(const Layout& layout, PreviewState& preview, FocusPane focus,
                 int inputKey) {
  if (!preview.session) {
    drawFrameBox(layout.middleTop, layout.rightLeft, layout.previewRows,
                 layout.previewCols, "", focus == FocusPane::Replay);
...
inline void drawLauncherKeyTips(const Layout& layout, FocusPane focus, MenuPage page) {
...
// Draws and drives the whole main-menu scene.
// The caller owns transitions between this launcher and gameplay/replay scenes.
inline LaunchRequest runLauncher(Settings& settings, std::string& roomCode,
