...
// get legal move.break list for a given state(can be in game rules or here,
// pending))
...
void AiPlayer::update(int ch, const GameState& state) {
  ...
  if (state.phase() == TurnPhase::Move && !m_moveReady) {
    if (m_difficulty == AiDifficulty::Easy)
      m_nextMove = findRandomMove(state);
    else if (m_difficulty == AiDifficulty::Medium)
      m_nextMove = findGreedyMove(state, true);
    else
      m_nextMove = findMinimaxMove(state, true);
    m_moveReady = true;
  } else if (state.phase() == TurnPhase::Break && !m_breakReady) {
    if (m_difficulty == AiDifficulty::Easy)
      m_nextBreak = findRandomBreak(state);
    else if (m_difficulty == AiDifficulty::Medium)
      m_nextBreak = findGreedyMove(state, false);
    else
      m_nextBreak = findMinimaxMove(state, false);
    m_breakReady = true;
  }
}
