...
MainMenuScene::MainMenuScene()
    : m_selectedOption(0), m_inDifficultyMenu(false), m_selectedDifficulty(0) {}
    ...
    if (!m_inDifficultyMenu) {
      switch (ch) {
        ...
        case 'S':
          ...
          if (m_selectedOption < 2) m_selectedOption++;
          break;
    ...
    // ---- DIFFICULTY SUBMENU INPUT ----
      ...
      switch (ch) {
        ...
        case 'S':
          ...
          if (m_selectedDifficulty < 3) m_selectedDifficulty++;
          break;
...
void MainMenuScene::drawMainMenu() {
  ...
  // ---- menu options ----
  std::vector<std::string> options = {"Human vs Human", "Human vs AI", "Quit"};
  ...
  // ---- controls hint ----
  std::string hint =
      "Arrow keys / WASD to navigate, Enter to select, Q to quit";
...
void MainMenuScene::drawDifficultyMenu() {
  ...
  std::vector<std::string> options = {"Easy   - Random moves",
                                      "Medium - Greedy strategy",
                                      "Hard   - Minimax AI", "Back"};
  ...
  // ---- hint ----
  std::string hint =
      "Arrow keys / WASD to navigate, Enter to select, ESC to go back";
  int hintCol = (maxX - (int)hint.length()) / 2;
