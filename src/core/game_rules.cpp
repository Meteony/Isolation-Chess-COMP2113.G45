...
bool GameRules::isLegalMove(const GameState& state, Side side, Coord dst) {
  ...
  if (!state.inBounds(dst)) return false;

  // dst intact
  if (state.tileAt(dst) != TileState::Intact) return false;

  // dst not be occupied by any player
  if (dst == state.playerPos(Side::Player1)) return false;
  if (dst == state.playerPos(Side::Player2)) return false;
...
bool GameRules::isLegalBreak(const GameState& state, Coord tile) {
  ...
  if (!state.inBounds(tile)) return false;

  // tile intact
  if (state.tileAt(tile) != TileState::Intact) return false;

  // tile not be occupied by any player
  if (tile == state.playerPos(Side::Player1)) return false;
  if (tile == state.playerPos(Side::Player2)) return false;
...
bool GameRules::hasAnyLegalMove(const GameState& state, Side side) {
  ...
  for (int r = currentPos.row - 1; r <= currentPos.row + 1; r++) {
    for (int c = currentPos.col - 1; c <= currentPos.col + 1; c++) {
      Coord target{r, c};
      if (isLegalMove(state, side, target)) return true;
    }
  }
...
bool GameRules::isTerminal(const GameState& state) {
  ...
  if (!hasAnyLegalMove(state, currentPlayer)) return true;
